---
description: the PM uses the result or reads the evidence and answers
...
2. This is not an entry command: it continues the record in flight (see `changes/QUEUE.md`) that is already at `deliver.accept`. Run `python3 framework/tools/orchestrate.py start <record> --command speckit.accept`; it refuses any record not at that step. `$ARGUMENTS`, if given, is the PM's note for this step.
