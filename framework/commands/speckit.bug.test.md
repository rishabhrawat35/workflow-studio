---
description: verify the fix; verified only if the failing criterion now holds
...
2. This is not an entry command: it continues the record in flight (see `changes/QUEUE.md`) that is already at `deliver.verify`. Run `python3 framework/tools/orchestrate.py start <record> --command speckit.bug.test`; it refuses any record not at that step. `$ARGUMENTS`, if given, is the PM's note for this step.
