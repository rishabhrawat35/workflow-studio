---
description: the PM approves the logic and confirms the criteria
...
2. This is not an entry command: it continues the record in flight (see `changes/QUEUE.md`) that is already at `define.approve`. Run `python3 framework/tools/orchestrate.py start <record> --command speckit.approve`; it refuses any record not at that step. `$ARGUMENTS`, if given, is the PM's note for this step.
