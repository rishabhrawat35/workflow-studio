---
description: an idea, sorted like any request. Spec Kit's assess pipeline (research, shape, decide) has no equivalent here; the discussion and the challenge do that work
...
2. This is an entry command: `python3 framework/tools/orchestrate.py start changes/<YYYY-MM-DD>-<n>-<slug>.md --command speckit.assess.intake`, with the PM's words (`$ARGUMENTS`) written under `## Request`.
