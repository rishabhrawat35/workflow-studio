---
description: re-plan: re-enters deliver.plan, which rewrites the plan and its task list (T001 [P] [node] grammar), then challenge-plan runs again
enters: deliver.plan
...
2. This is not an entry command: it continues the record in flight (see `changes/QUEUE.md`) that is already at `deliver.plan`. Run `python3 framework/tools/orchestrate.py start <record> --command speckit.tasks`; it refuses any record not at that step. `$ARGUMENTS`, if given, is the PM's note for this step.
