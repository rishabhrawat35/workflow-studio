---
description: challenge the draft node. Note: Spec Kit's checklist writes a requirements checklist for the human to tick; ours runs the challenger and returns findings
...
2. This is not an entry command: it continues the record in flight (see `changes/QUEUE.md`) that is already at `define.challenge`. Run `python3 framework/tools/orchestrate.py start <record> --command speckit.checklist`; it refuses any record not at that step. `$ARGUMENTS`, if given, is the PM's note for this step.
