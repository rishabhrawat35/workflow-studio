---
description: ask for a change to one architecture rule; sorted to Foundation, where the decision behind the current rule is brought forward
...
2. This is an entry command: `python3 framework/tools/orchestrate.py start changes/<YYYY-MM-DD>-<n>-<slug>.md --command speckit.rule`, with the PM's words (`$ARGUMENTS`) written under `## Request`.
