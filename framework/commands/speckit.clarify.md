---
description: ask the open questions on the current draft: max three, one at a time, each with a recommended option
enters: define.write
...
2. This is not an entry command: it continues the record in flight (see `changes/QUEUE.md`) that is already at `define.write`. Run `python3 framework/tools/orchestrate.py start <record> --command speckit.clarify`; it refuses any record not at that step. `$ARGUMENTS`, if given, is the PM's note for this step.
