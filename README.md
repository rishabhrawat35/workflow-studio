...
**The PM describes what the product should do; the AI sorts, drafts, challenges its own draft, plans, builds and verifies; and `orchestrate.py` refuses to move a change past a gate without the artifact that gate requires.**

![Python 3.10+](https://img.shields.io/badge/python-3.10%2B-blue) ![Works with 5 AI tools](https://img.shields.io/badge/works%20with-Claude%20Code%20%C2%B7%20Codex%20%C2%B7%20Copilot%20%C2%B7%20Gemini%20%C2%B7%20Cursor-informational) ![Status](https://img.shields.io/badge/status-working%2C%20unreleased-orange) ![Test cases](https://img.shields.io/badge/test%20cases-207-brightgreen)

**Status:** working, unreleased; on 2026-09-10 `python3 tools/check_all.py` printed `ALL CHECKS PASSED`.
...
The workflows live in `workflows/*.yaml`, `framework/tools/orchestrate.py` enforces them, and [studio.html](studio.html) draws them; a product repository copies `framework/` in and runs the first command below.
...
> The AI agent operating this framework inside a product repository reads [For AI agents](#for-ai-agents), then `AGENTS.md` and `framework/README.md` in that repository.
...
- **A tool enforces the gates.** `orchestrate.py advance` is the only way a record moves; the [worked example](#worked-example-a-refund-within-7-days) shows six real refusals.
- **The PM never reads code.** The node, the record, the plan section and the evidence at acceptance are plain language.
- **Every draft is challenged first.** The architecture file, the node, the plan and the code each receive a second AI pass in a fresh context ([challenger-pass.md](framework/components/challenger-pass.md)); zero findings on the full lane are refused while a framing is unused.
- **State lives in one file per request.** The record's front matter holds the step, the lane and every edge taken, so any AI resumes with `orchestrate.py next`.
- **Decisions are brought forward.** A gate answer with an alternative becomes a file under `decisions/` ([decision-log.md](framework/components/decision-log.md)) and is shown to the PM before a draft alters it.
- **Commands carry the command vocabulary of [GitHub Spec Kit](https://github.com/github/spec-kit).** `/speckit-specify`, `/speckit-plan` and `/speckit-implement` land in the folders Spec Kit uses, so a move either way is a file swap.
...
Every request enters at `define.intake`; the three PM gates are `define.approve`, `deliver.approve-plan` and `deliver.accept`.
...
No package exists: a product repository holds a copy of `framework/`. The framework needs five things:

| Requirement | Why |
|---|---|
| Python 3.10 or newer | `tools/validate.py` uses `str \| None` annotations |
...
`install_commands.py` writes the 21 templates from [framework/commands/](framework/commands/) into the chosen tool's folder.
...
`--agent all` installs all five layouts, 105 files. The PM fills in the two commands left open in `AGENTS.md`: how to run the product and the test suite.

## Quickstart

1. Install the framework and one tool's commands as shown under [Installation](#installation).
2. Describe the architecture with `/speckit-constitution Node 20 + Fastify backend under code/backend, Postgres on Supabase. Run with npm run dev. Never touch migrations/ by hand.`; the PM then writes the glossary.
...
5. Run `/speckit-plan`; the AI stops at `deliver.approve-plan` for `execute`, `revise the plan`, `revise the logic` or `drop`.
6. Try the running product and answer `/speckit-accept accepted`.
7. Run `/speckit-git-commit` for the one commit that carries code, tests and the record.

## Worked example: a refund within 7 days

The slash commands run these commands for the AI; they ran on 2026-09-10 in a fresh product folder, and every output block is verbatim. Four more requests reach the same machinery through these commands:
...
After exit 3 of `define.sort` and the PM's notes at `define.discuss`, the draft reached `define.challenge`.
...
The record passed `deliver.plan`, `deliver.challenge-plan`, `deliver.approve-plan` (`execute`), `deliver.execute`, `deliver.verify` and `deliver.passes`; the PM answered `accepted` at `deliver.accept`, and `commit.py` committed the delivery's files only.
...
Command names follow [GitHub Spec Kit](https://github.com/github/spec-kit) where the meaning overlaps; eight entry commands create a record at `define.intake`, and every other command continues a record already at its step or is refused ([commands.yaml](framework/orchestration/commands.yaml)).

<details>
<summary>The 21 slash commands</summary>
...
- **Each step has one role and one mode.** [roles.yaml](framework/orchestration/roles.yaml) maps the 51 steps to six roles and four modes; a role is a fresh context reading only the step's inputs.
- **Each term the PM reads has one definition file.** The 12 files under [framework/components/](framework/components/) share five sections, from Definition to Place in the system.
- **The renderer works offline.** `studio.html` embeds the workflows as JSON.
...
`tools/check_all.py` proves this repository in one run and stops at the first failure, in this order:
...
6. The generators and the installer run on a fixture whose broken decision file must be refused.
...
```
   wrote decisions/INDEX.md (2 decisions, 2 finding(s))
     - D-0002-bad.md: made_at must be a quoted ISO timestamp with time and offset, e.g. "2026-09-09T14:32+05:30"
     - D-0002-bad.md: supersedes unknown decision D-0099

ALL CHECKS PASSED
```

[framework/test-cases.md](framework/test-cases.md) is the regression suite for the workflows themselves: 207 cases in sections A–M and O–R, each with a verdict and, where it passes, its step path; section N re-runs every case against the current workflows.
...
The agent takes the **AI** role; the PM is the only human.
...
3. Run `python3 framework/tools/orchestrate.py next changes/2026-09-10-1-refund-unused-policy.md`, do exactly the step it names, and write the output into the record.
...
5. Read a `refused:` line as the name of a missing artifact or the only legal exit, never as an error to work around.
6. Stop and show the PM the question with the YAML's options whenever `next` shows `mode: pm`.
7. As a challenger, read only the listed inputs and return findings only.

No code is written before `deliver.approve-plan` says `execute`, no node is saved before `define.approve`, and no record is marked done before `deliver.accept`.

## Customizing the workflows

The YAML is the source of truth; `studio.html` and `framework/steps.md` are generated from it and never edited by hand.
...
- The framework is not a deployment pipeline: a change ends at a commit the PM has seen work.
- The framework is not a multi-approver system: one PM, one AI.
- The framework is not a spec-per-feature tool: the whole product is one logic tree.
- `orchestrate.py` records what the AI writes but does not judge it; a one-line findings block satisfies the gate.
...
| [framework/test-cases.md](framework/test-cases.md) | The regression suite of 207 cases |
...
The repository is public at [github.com/rishabhrawat35/workflow-studio](https://github.com/rishabhrawat35/workflow-studio) and has no CONTRIBUTING.md yet. A bug report is an issue carrying the output of `python3 tools/check_all.py`; a workflow change follows [AGENTS.md](AGENTS.md).

## Acknowledgements

[GitHub Spec Kit](https://github.com/github/spec-kit) supplied the command vocabulary and the skills layout; [Rira](https://github.com/rishabhrawat35) built the rest with Claude in September 2026.
